...
void loop() {
  ...
  while ( millis() - tempo < intervalo) {
    ...
    //caso o botão das horas for pressionado a mesma será incrementada
    bhrs = digitalRead(phrs);
    if (bhrs == 0) {
      hrs++;
      while (bhrs == 0) {
        bhrs = digitalRead(phrs);
        Serial.println("botão das horas pressionado");
        Serial.println(bhrs);
        //para evitar que as horas sejam imcrementados enquanto o botão for pressionado apenas uma vez
      }
    }
    //caso o botão dos minutos for pressionado o mesmo será incrementado
    bmin = digitalRead(pmin);
    if (bmin == 0) {
      minu++;
      while (bmin == 0) {
        bmin = digitalRead(pmin);
        Serial.println("botão dos minutos pressionado");
        Serial.println(bmin);
        //para evitar que os minutos sejam incrementados mais de uma vez enquanto o  botão for pressionado apenas uma vez
      }
    }
